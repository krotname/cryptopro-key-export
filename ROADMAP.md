...
- [ ] **E2E-тест на физическом Рутокене S.** Вставить токен, прогнать `CryptoProExport.exe full <dir>`.
      ...
      **Без железа не закрыть.** Всё, что после снятия с токена, уже проверено на синтетическом контейнере.
- [ ] **Рутокен Lite через PKCS#11** (`rtPKCS11ECP.dll`) — единственный путь к контейнерам на Lite,
      раз rtCOMLite их не отдаёт. Раньше числилось в «идеях», теперь это условие поддержки заявленного устройства.
