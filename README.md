...
| **Рутокен Lite** | смарт-карта | не удалось ❌ | живой токен: `rtCOMLite` 0 файлов, PKCS#11 0 объектов. APDU-уровень (свой PC/SC-стек CSP) не пробовали |
| **Рутокен ЭЦП / ЭЦП 2.0** | чип смарт-карты | нет ❌ | живой токен (12.08.2026): `rtCOMLite` 19 папок / 0 файлов; PKCS#11 `CKA_EXTRACTABLE=false` (аппаратный флаг чипа); `certmgr -export -pfx` и `csptest -keycopy` → `0x8009000b` |
...
- **сертификат** извлекается прямо с токена без КриптоПро CSP (`token [outDir]` / кнопка
  «Извлечь сертификат») — **на Рутокен ЭЦП**, где он лежит публичным объектом PKCS#11.
  На Рутокен Lite объектов PKCS#11 нет, поэтому этой командой с него брать нечего;
...
неизвлекаем; при его утере ключ восстановлению не подлежит, порядок обычный — отзыв
сертификата и выпуск нового в удостоверяющем центре. Для Lite вопрос открыт на уровне APDU,
для Рутокен S путь заложен и ждёт проверки на живом устройстве.
