...
- Снятие с токена рассчитано на Рутокен S (файловый контейнер в памяти токена).
  На **Рутокен Lite** проверено на живом устройстве: rtCOMLite показывает структуру каталогов,
  но самих файлов контейнера не отдаёт, поэтому снять контейнер с Lite этим путём нельзя.
  PKCS#11 рассматривался как замена и **не подошёл**: `rtPKCS11ECP.dll` не показывает на этом
  токене ни одного объекта, то есть контейнеры КриптоПро там не PKCS#11-объекты. Штатное
  копирование контейнера средствами CSP тоже отпадает — оно упирается в запрет на экспорт ключа.
  Известные проверки и что осталось — в [ROADMAP.md](ROADMAP.md). Рутокен ЭЦП 2.0 с аппаратным неизвлекаемым
  ключом не выгрузить в принципе. Остальной конвейер — снятие запрета, установка в CSP,
  экспорт в PFX — работает с контейнером, который уже лежит папкой на диске.
