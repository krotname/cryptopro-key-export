...
`header.key`) не мешает — в этом весь смысл инструмента. Если ключ обслуживается по
смарт-карточному профилю, эта файловая память через `rtCOMLite` не видна, и снять ключ
проверенными способами не удаётся.
...
| **Рутокен Lite** | смарт-карта | не удалось ❌ | живой токен (fw 9.02): `rtCOMLite` 0 файлов; контейнеры CSP объектами PKCS#11 не становятся. APDU-уровень (свой PC/SC-стек CSP) не пробовали |
...
Разница в доказательности между Lite и ЭЦП: на ЭЦП ключ помечен аппаратно неизвлекаемым в
самом чипе (`CKA_EXTRACTABLE=false`), это свойство носителя. На Lite проверенные пути
(`rtCOMLite`, PKCS#11, `csptest`) ключ не отдают, но КриптоПро работает с ним через
собственный PC/SC-стек на уровне APDU — этот путь не исследован, поэтому «невозможно»
сказать пока нельзя, только «проверенными способами не снимается».
...
отзыв сертификата и выпуск нового в удостоверяющем центре. Для Lite вопрос открыт на уровне
APDU, для Рутокен S путь заложен и ждёт проверки на живом устройстве.
...
- **Снятие закрытого ключа с токена** рассчитано на Рутокен S (файловый контейнер в памяти
  ...
  rtCOMLite не видна (проверено на живых устройствах), и проверенными способами закрытый ключ
  с них не снять. Подробная матрица «модель → снимается ли ключ», с разницей между Lite и ЭЦП,
...
CryptoProExport.exe extractpfx <folder> <out.pfx> <pfxPass> [password] [cert.cer]  # .pfx без CSP
