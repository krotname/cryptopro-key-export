...
| **Рутокен ЭЦП 3.0** | чип смарт-карты (ФКН) | нет ❌ | живой токен (13.08.2026, fw 30.02) был пуст, поэтому извлечение не пробовали; проверено обратное: КриптоПро видит носитель как ФКН (`rutoken_fkc_…`) и ключ с диска на него **не пишет** (`0x80090029`), хотя на Lite и в HDIMAGE то же копирование проходит |
