...
| **Рутокен ЭЦП 3.0** | чип смарт-карты (ФКН) | нет ❌ | **проверено прямым опытом** (13.08.2026, fw 30.02): на токене создан контейнер с двумя **неэкспортируемыми** ГОСТ-ключами (`KP_PERMISSIONS` без `CRYPT_EXPORT`), ни один путь снятия не сработал — PKCS#11 чтение `CKA_VALUE` и `C_WrapKey` (ключ `CKA_NEVER_EXTRACTABLE=true`), `rtCOMLite` 0 файлов, `csptest -keycopy` токен→диск и `certmgr -export -pfx` → `0x8009000b`. Плюс носитель — ФКН, ключ с диска на него не пишется (`0x80090029`) |
| **JaCarta** (Аладдин Р.Д.) | смарт-карта | **не проверялось** | живой экземпляр 13.08.2026 (JaCarta PRO, модель PKCS#11 `PRO`, fw 1.01): PIN на нём **не заводской**, поэтому ничего требующего авторизации не пробовали — счётчик попыток чужого носителя дороже опыта. Без PIN видно: контейнер КриптоПро на нём есть и виден CSP, а PKCS#11 (`jcPKCS11-2.dll`) не показывает ни одного объекта — та же картина, что на Lite |
