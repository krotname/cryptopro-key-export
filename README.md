...
программа его не копирует: файлового контейнера `*.key` для этого пути нет. Атрибуты
`CKA_EXTRACTABLE`/`CKA_NEVER_EXTRACTABLE` относятся к конкретному объекту ключа и не выводятся
из PID, ATR, строки модели или версии прошивки.
...
- **диагностика**: тип, серийник, состояние PIN (без траты попыток), версия аппаратуры,
  механизм-профиль и публичные объекты, если они действительно присутствуют;
...
1. *Обновить список* / `list` — показать CSP-контейнеры и все токены из PKCS#11; строки `[PKCS#11]` и `[rtCOMLite]` в журнале и CLI имеют отдельные счётчики.
