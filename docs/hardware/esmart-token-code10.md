# ESMART Token VID_2CE4/PID_7479: Code 10 и окончательное закрытие

Ниже сохранён диагностический baseline 21.08.2026. Стойких системных изменений тогда не было, кроме
...
## Актуальный статус 27.08.2026

Проблема закрыта повторным аппаратным readback и полным функциональным тестом:

- PnP: `Status=OK`, `ProblemCode=0`; reader стабильно доступен как
  `ISBC ESMART Token 0`;
- установлен официальный ESMART PKI Client 4.17.4, системные x86/x64 пары
  `isbc_pkcs11_main.dll` + `isbc_esmart_token_mod.dll` найдены и загружаются;
- PKCS#11 на точном носителе прошёл создание/чтение/удаление объектов и RSA-2048
  sign/verify; аппаратный private key подтвердил `CKA_NEVER_EXTRACTABLE=true`;
- КриптоПро создал на токене синтетический двухключевой файловый контейнер;
- production `tokenfull` прочитал все шесть файлов по ESMART APDU, снял запрет для
  обеих пар, HDIMAGE-копия получила `0x0013089C`/`0x0012289C`, PFX подтверждён OpenSSL.

То есть Code 10 не является текущим ограничением модели. Старый раздел ниже объясняет
первопричину наблюдавшегося failed-start и остаётся полезен для диагностики повторения.
Полный протокол — [esmart-usb64k.md](esmart-usb64k.md).
