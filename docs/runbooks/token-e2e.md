...
- **`<tag>` — только `[A-Za-z0-9_]`.** Тег подставляется в `-match`-шаблоны (резерв
  в §3, чистка сертификатов в §7c); метасимвол regex (`.`, `+`, `[`…) сматчил бы
  или пропустил не тот сертификат. Если нужен произвольный тег — оборачивать в
  `[regex]::Escape()`.
...
# ВНИМАНИЕ: сам по себе enum_cont ненадёжен как «доказательство отсутствия» —
# под contention/после холодного сброса он молча пропускает reader (AGENTS п.31),
# и одноимённый контейнер на «онемевшем» соседнем токене в $csp не попадёт.
# Поэтому либо (надёжнее всего) на время прогона держать подключённым ТОЛЬКО целевой
# токен, либо доказать, что опрошен каждый reader: все считыватели из `list`/PKCS#11
# должны присутствовать в $csp; если какой-то «онемел» — холодный сброс и повтор.
