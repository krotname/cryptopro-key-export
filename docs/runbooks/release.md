...
пункты — они пойдут в раздел «Текущее состояние» ROADMAP, а после публикации
(шаг 6) — в заметки самого релиза. Тело релиза workflow ставит постоянное
(описание портативной сборки и SHA-256) и списка изменений в нём нет.
...
Самопроверка **скачанного** файла: релиз проверяется тем же
...
## 6. Дописать список изменений в заметки релиза

Workflow отправляет в API постоянный текст (портативная сборка, требования,
SHA-256) — списка изменений там нет и быть не может: он не знает, что вошло.
Пункты из шага 2 добавляются к готовым заметкам после публикации, чтобы не
потерять сумму и описание:

```powershell
$notes = "$env:TEMP\cpx-rel\notes.md"
gh release view v1.8.0 --json body --jq .body | Set-Content $notes -Encoding utf8
...
- ...
'@
gh release edit v1.8.0 --notes-file $notes
gh release view v1.8.0 --json body --jq .body    # перечитать, что получилось
```
...
- **Тег поставлен по ошибке** — снять релиз и тег во всех трёх местах, поднять
  версию и повторить. Локальный ref обязателен: без него `git push --tags` вернёт
  отозванный тег и снова запустит выпуск, а повторное использование той же версии
  упрётся в «tag already exists».

  ```powershell
  gh release delete v1.8.0 --yes --cleanup-tag   # релиз и тег на GitHub
  git push origin :refs/tags/v1.8.0              # если тег остался на origin
  git tag -d v1.8.0                              # локальный ref
  ```
