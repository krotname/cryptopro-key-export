...
Актуальная регрессия: [два PRO от 14.09.2026](../hardware/purple-pro-regression-20260914.md).
Начиная с v1.9.0 `tokenfull` сохраняет обе пары в одной папке и проверяет
их назначение по отдельным отпечаткам заголовка.

Ниже исторический протокол от 27 августа 2026 года. Документ описывает только
...
Реализация `JaCartaProApdu` **на тот момент** принимала только 20 байт и останавливалась до
защищённых `READ BINARY`; выходная папка осталась пустой, а повторный `checkexport`
показал те же `…98`. Это несовместимость формата ответа соли с тогдашним декодером, а
...
добавочный транспортный хвост. Контрольный challenge-response на том же exact-target
показал, что первые 20 байт payload дают `0x9000`, а передача всех 43 байт в KDF
даёт `0x6300`. Для этой ревизии первые 20 байт являются солью KDF, а назначение
дополнительных 23 байт не утверждается и в KDF не передаётся. Совместимый
декодер должен принимать 20-байтовый ответ старой ревизии и 43-байтовую запись
текущей ревизии, извлекая первые 20 байт без усечения произвольного участка.

### ИСПРАВЛЕНО — декодер соли (PR #108) и живой E2E 15.09.2026

Баг декодера соли устранён коммитом `e69f0e5` («Fix JaCarta PRO 43-byte salt record», PR #108):
`JaCartaProApdu.ExtractServiceSalt` теперь принимает и 20-байтовый ответ старой ревизии, и
43-байтовую запись текущей PRO-ревизии, извлекая из неё первые 20 байт для legacy-KDF; прочие
длины отвергаются как `SERVICE_SALT_LENGTH`. Регрессия закрыта тестом
`tests/CryptoProExport.Tests/JaCartaProSaltTests.cs` (20/43 → первые 20 байт; 0/19/44 → отказ).

Проверено вживую 15.09.2026 на эталонном eToken PRO **PROFELTORG** (`Aladdin Token JC 1`, серийный
`00A7A257`, заводской PIN `1234567890`): на свежесозданном CSP-контейнере `cpxt_verify_20260915`
production-команда `tokenexport … --container jacartapro_01` завершилась кодом `0` и сняла по APDU
без CryptoPro все 6 файлов контейнера (`masks/primary/header/masks2/primary2/name.key`) — соль 43
байта декодировалась штатно. Контейнер после проверки удалён.
