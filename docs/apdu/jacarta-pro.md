...
Перед изменением KDF нужно определить структуру 43-байтового ответа и подтвердить её
на том же exact-target. Простое усечение до 20 байт недопустимо: оно может дать
неверный challenge-response и расход PIN-попытки.
