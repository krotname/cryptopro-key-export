...
namespace CryptoProExport
{
    ...
    public static class CryptoProHeaderExportability
    {
        ...
        private static Asn1Sequence RewriteContent(Asn1Sequence content, bool exchange, bool signature)
        {
            ...
            //
            // Когда пары две, бит взводится в обоих наборах, и раскладка «какой набор чей» на
            // результат не влияет — так и проверено с КриптоПро. Для контейнера с одной парой,
            // у которого при этом есть поле [4], соответствие взято из Android-ядра и живым CSP
            // не подтверждено: такой контейнер пока не попадался.
