...
13. **Локальный запуск build-версии требует 32-битного рантайма .NET.** Сборка даёт framework-dependent x86; без `Microsoft.WindowsDesktop.App` x86 exe падает с `0x80008002`. На CI поэтому самопроверка гоняется на портативной (self-contained) сборке.
