...
14. **Руководство лежит в `src/Core/guide.txt`** и вшивается ресурсом (`GuideText`). Оно же — окно «Справка» и команда `help`. Тесты следят за наличием разделов, упоминанием всех команд и шириной строк ≤78 символов (руководство печатается в консоль как есть). Добавил команду — допиши её в guide.txt, иначе тест упадёт.
